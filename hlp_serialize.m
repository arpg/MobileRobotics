...
%                                (C) 2010 Tim Hutt
    global vSerialzedHandleObjects;
    vSerialzedHandleObjects = {};
    m = hlp_serialize_(v);
end

function m = hlp_serialize_(v)
    global vSerialzedHandleObjects;
    ...
    elseif isobject(v)
        if 0
        for ii = 1:size(vSerialzedHandleObjects,2)
            if v == vSerialzedHandleObjects{ii}
                m = hlp_serialize_([]);
           %     m = serialize_object( placeholder(class(v)) ); % serialize a dummy stub
                return;
            end
        end
        if isa(v,'handle')
            vSerialzedHandleObjects{size(vSerialzedHandleObjects,2)+1} = v;
        end
        end
...
function m = serialize_cell_heterogenous(v)
    contents = cellfun(@hlp_serialize_,v,'UniformOutput',false);
    ...
    elseif isempty(v)
        ...
        elseif (size1+size2 == 0) & (dims == 2) %#ok<AND2>
            ...
            elseif length(unique(cellfun(@class,v(:),'UniformOutput',false))) == 1
                ...
                m = [uint8(38); hlp_serialize_(class(v{1})); ndims(v); typecast(uint32(size(v)),'uint8').'];
            else
