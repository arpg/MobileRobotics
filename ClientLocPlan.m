%% usage: 
%% close all; clear all; ClientLocPlan('Yourname','r',8888,'DilatedMap.png');
function ClientLocPlan( sName, sColor, ProxyPortNum, sMap )
    Map=imread(sMap);
    ...
    while 1
        ...
        if norm(PoseTarget(1:2)-CurrPose(1:2))< 1 % robot is in target point
            PoseTarget = ginput(1);  % get new target
        end
        ...
        N_particles_ = 500;
        threshold_ = 0.1;
        CurrPose = ParticleFiltering(z,PrevCommand,laser.nbeams,laser.m_dMaxRange,laser.fov,Map,N_particles_,threshold_)
        % 2) Plan
%         p = ScanToPoints( z, xwr, laser.fov, laser.nbeams );
        PotFieldMap = PotentialFields(PoseTarget,Map);
        NextComm = GenerateCommands(Potfield, CurrPose, m_MaxAbsLinearVelocity, m_MaxAbsAngularVelocity);
%         delete(h);
...
%        ApplyCommandNew( robot,u );
       ApplyCommandNew( robot,NextComm );
       PrevCommand = NextComm;
